"""Based on 10_Assembled_Outcome_v2.py
Improving program based on user feedback
Clearing the entry box in the Quiz class after the user has pressed next
Added text to say answer
Added self.report_string that takes self.user_name and the number of
correct and incorrect answers to write into the exported file.
Lines 45-48,146-152, 212-214,240, 543-564, other changes to 569-581
Nathan Morrison
6/09/2021
"""
...
class Quiz:
    def __init__(self, partner, mode):
        ...
        self.question_label = Label(
            self.quiz_frame, text=f"What is the {mode} of "
                                  f"{self.main_dict.get(self.key)}",
            font="Arial 12", wrap=250, pady=10, bg=background_color)
    ...
    def check_button_commands(self, partner):
        ...
        if location_find:
            ...
            if self.main_dict.get(location_find) == self.key_value:
                ...
                self.question_label.configure(
                    text=answer, fg="red")
                self.answer_label.config(text=f"The correct answer was "
                                              f"{self.key}")
                partner.incorrect += 1
    ...
    def next_command(self, mode):
        ...
        self.answer_label.config(text=self.answer_label_text)
