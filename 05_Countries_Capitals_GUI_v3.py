"""Based on 05_Countries_Capitals_GUI_v2.py
Copying code from 'capitals' class and reusing it for the 'countries' class
which will have the same function. Any variables with the name 'capitals' will
...
class Countries:
    def __init__(self, partner):
        background_color = "DodgerBlue2"

        # to disable countries and capitals buttons when quiz is open.
        partner.countries_button.config(state=DISABLED)
        partner.capitals_button.config(state=DISABLED)

        # Sets up a child window for countries
        self.countries_box = Toplevel()

        # If a user presses the red cross at the top, it will use close_quiz
        # This closes the quiz window and enables the capitals and countries
        # buttons in the main menu.
        self.countries_box.protocol('WM_DELETE_WINDOW',
                                    partial(self.close_quiz, partner))

        # Countries GUI
        self.countries_frame = Frame(self.countries_box, width=375, height=300,
                                     bg=background_color)
        self.countries_frame.grid()

        # Countries heading (row 0)
        self.countries_label = Label(self.countries_frame,
                                     text="Countries", font="Arial 18 bold",
                                     bg=background_color, padx=10, pady=10)
        self.countries_label.grid(row=0)

        # Question label (row 1)
        self.question_label = Label(self.countries_frame,
                                    text="<ask question here>",
                                    font="Arial 12", wrap=250, pady=10,
                                    bg=background_color)
        self.question_label.grid(row=1)
        ...
        # Answer entry box label (row 2)
        self.answer_label = Label(self.countries_frame,
                                  text="Type your Answer and press 'Check' "
                                       "to enter", font="Arial 10 italic",
                                  bg=background_color)
        self.answer_label.grid(row=2, padx=30)
        # Answer entry box (row 3)
        self.get_answer_entry = Entry(self.countries_frame, width=17,
                                      font="Arial 12")
        self.get_answer_entry.grid(row=3)

        # Buttons Frame (row 4)
        self.buttons_frame = Frame(self.countries_frame, pady=10, padx=10,
                                   width=200, bg=background_color)
        self.buttons_frame.grid(row=4)
