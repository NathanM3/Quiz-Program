"""Based on 10_Assembled_Outcome_v4.py
adding .title() function to the name check functions from Menu class and Quiz
separating score text and answer text (line 543-557
8/09/2021
"""
...
class Menu:
    def __init__(self):
        ...
        background_color = "#89B4E5"  # Colour from hexadecimal colour website

        # List to hold history of answers given by user.
        self.all_answers = []
        self.user_name = ""  # self.user_name to be called later
        self.report_string = ""  # self.report_string can be called later
        self.correct = 0  # self.correct keeps track of correct answers
        self.incorrect = 0  # self.incorrect keeps track of wrong answers
        # Main Menu GUI - uses the chosen background colour (light blue)
        self.main_menu_frame = Frame(width=375, height=300,
                                     bg=background_color)
        self.main_menu_frame.grid()

        # Main Menu heading (row 0) - inside main menu frame and light blue
        self.main_menu_label = Label(
            self.main_menu_frame, text="Main Menu", font="Arial 18 bold",
            bg=background_color, padx=10, pady=10)
        self.main_menu_label.grid(row=0)

        # Get name label (row 1) - small instructions to help user.
        self.get_name_label = Label(
            self.main_menu_frame, text="Enter your name and press one of "
            "the buttons below", font="Arial 10 italic", wrap=250, pady=10,
            bg=background_color)
        self.get_name_label.grid(row=1)

        # Name entry box (row 2) - Entry box for user name
        self.get_name_entry = Entry(self.main_menu_frame, width=17,
                                    font="Arial 12", justify=CENTER)
        self.get_name_entry.grid(row=2)

        # Buttons Frame - Top button frame holds the quiz mode buttons
        ...
        self.countries_button = Button(
            self.buttons_frame, text="Countries", font="Arial 14 bold",
            width=10, bg="blue", fg="snow", command=lambda: self.quiz(
                "country", "capital"))
        ...
        self.capitals_button = Button(
            self.buttons_frame, text="Capitals", font="Arial 14 bold", width=10
            , bg="forest green", fg="snow", command=lambda: self.quiz(
                "capital", "country"))
        ...
        self.history_button = Button(
            self.buttons_frame,text="Answer Record History", wrap=150,
            font="Arial 12 bold", bg="light grey", width=12,
            command=self.history, state=DISABLED)
    ...
    # function to make sure string only contains certain characters.
    def name_check(self):
        valid_char = "[A-Za-z ]"  # Capitals and Lower case, (and spaces).
        while True:  # Loop does not end
            name = str(self.get_name_entry.get()).title()  # from entry box
            try:
                for letter in name:
                    if not re.match(valid_char, letter):
                        ...
                        raise NamingError
                if len(name) < 3 or "  " in name:  # other rules for string
                    raise NamingError
    ...
    # classes
    def quiz(self, mode, anti_mode):
        self.user_name = self.name_check()
        if self.user_name:
            get_quiz = Quiz(self, mode, anti_mode)

    def help(self):
        get_help = Help(self)  # Opens Help GUI

    def history(self):
        self.report_string = f"{self.user_name} had {self.correct} correct " \
                             f"answer(s)\n{self.incorrect} incorrect answer" \
                             f"(s)\nThis is a score of " \
                             f"{self.correct/self.incorrect}"
        self.all_answers.insert(0, self.report_string)
        get_history = History(self, self.all_answers)


class History:
    def __init__(self, partner, answers):
        background = "SlateGray1"  # A light grey colour with a hint of blue
        # disable history and quiz buttons in main menu.
        ...
        self.history_box.protocol('WM_DELETE_WINDOW',
                                  partial(self.close_history,
                                          partner))
        # set up GUI Frame
        self.history_frame = Frame(self.history_box, width=600, bg=background)
        ...
        self.history_text = Label(
            self.history_frame,
            text="Here are your most recent answers. Please use the export "
                 "button to create a text file of all your answers "
                 "for this session", font="arial 10 italic", justify=LEFT,
            padx=10, pady=10, bg=background, fg="maroon", wrap=250)
        self.history_text.grid(row=1)
        # History Output goes here... (row 2)
        history_string = ""
        # orders items in list to be on separate lines.
        if len(answers) >= 7:
            for item in range(0, 7):
                history_string += answers[
                                      len(answers) - item - 1] + \
                                  "\n"
        else:
            for item in answers:
                history_string += answers[len(
                    answers) - answers
                    .index(item) - 1] + "\n"
                self.history_text.config(
                    text="Here is your answer history. You can use the export "
                         "button to save this data to a text file if desired.")

        # Label to display answer history to user
    ...
    def close_history(self, partner):
        ...
        # prevents duplicate user names entered into list.
        partner.all_answers.remove(partner.report_string)
        self.history_box.destroy()

    # opens the export class GUI. Also takes answer history for text file.
...
class Export:
    def __init__(self, partner, answer_history):
        background = "#A3CAA4"  # Pale colour from a colour mixer online
        ...
        self.export_text = Label(
            self.export_frame,
            text="Enter a filename in the box below and press the Save button "
                 "to save your answer history to a text file.",
            justify=LEFT, width=50, bg=background, wrap=300)
        self.export_text.grid(row=1)

        # Warning text (label, row 2)
        self.export_text = Label(  # pink background colour
            self.export_frame,
            text="If the filename you enter below already exists, it's conte"
                 "nts will be replaced with your answer history", justify=LEFT,
            bg="#ffafaf", fg="maroon", font="Arial 10 italic", wrap=400,
            padx=10, pady=10)
...
class Help:
    def __init__(self, partner):
        ...
        self.help_text = Label(
            self.help_frame,
            text="This is a Geographical Quiz that will show you the name of "
                 "a country or capital and ask you to enter the corresponding "
                 "capital or country.\nAfter you have completed the quiz or "
                 "decided to stop playing, a record of answers can be expor"
                 "ted by pressing the 'Answer Record History' button and foll"
                 "owing the instructions given.\nEnjoy", justify=LEFT,
                 width=40, bg=background, wrap=250)
        self.help_text.grid(row=1)
        # Dismiss button (row 2)
        self.dismiss_btn = Button(
            self.help_frame, text="Dismiss", width=10, bg="slate gray",
...
# The Quiz class is what runs the quiz game.
class Quiz:
    def __init__(self, partner, mode, anti_mode):
        self.mode = mode
        ...
        self.answer_label_text = "Type your Answer and press 'Check' to enter"
        # More may be added to these as the quiz is developed later on.
        background_color = "chartreuse4"  # default color for capitals
        heading_text = mode.title()  # mode is either "capitals" or "countries"
        ...
        # setting the text to be written for the questions based the mode and
        # the anti_mode (if mode is "country" anti_mode is "capital")
        self.question_text = f"What {mode} matches with this {anti_mode}?: " \
                             f"\n{self.main_dict.get(self.key)} "
        ...
        self.quiz_frame = Frame(self.quiz_box, width=500, height=300,
                                bg=background_color)
        ...
        self.question_label = Label(
            self.quiz_frame, text=self.question_text,
            font="Arial 12", wrap=300, pady=10, bg=background_color)
        self.question_label.grid(row=1, padx=10)

        # Answer entry box label (row 2) - Instructions written in small text
        self.answer_label = Label(
            self.quiz_frame, text=self.answer_label_text,
            font="Arial 10 italic", wrap=250, bg=background_color)
        self.answer_label.grid(row=2, padx=10)
        ...
        self.check_button = Button(
            self.buttons_frame, text="Check", font="Arial 14 bold", width=8,
            bg="sandy brown", fg="black", command=lambda:
            self.check_button_commands(partner))
        ...
        self.next_button = Button(
            self.buttons_frame, text="Next", font="Arial 14 bold", width=8,
            bg="firebrick2", fg="snow", state=DISABLED, command=lambda:
            self.next_command(mode))
        self.next_button.grid(row=0, column=1, padx=5, pady=10)
        # Dismiss button to close window.
        self.dismiss_btn = Button(
            self.buttons_frame, text="Dismiss", width=8, bg="slate gray",
            font="arial 14 bold", command=partial(self.close_quiz, partner),
            fg="white")

        self.dismiss_btn.grid(row=0, column=2, pady=10, padx=5)

    # similar to the name_check from main menu, changes to valid chars
    # and configurations to labels etc.
    def name_check(self):
        valid_char = "[A-Za-z ']"
        while True:  # Loop does not end
            name = str(self.get_answer_entry.get().title())
            try:
                for letter in name:
                    if not re.match(valid_char, letter):
    ...
    def check_button_commands(self, partner):
        ...
        if location_find:
            ...
            if self.main_dict.get(location_find) == self.key_value:
                partner.correct += 1
                answer = f"'{location_find}' is the {self.mode} of " \
                         f"{self.key_value}"
                score_text = f"\n{partner.correct} correct " \
                             f"answer(s) | {partner.incorrect} incorrect " \
                             f"answer(s)"
                self.question_label.configure(
                    text=answer+score_text, fg="white")

            else:
                partner.incorrect += 1
                answer = f"'{location_find}' is not the {self.mode} of " \
                         f"{self.key_value}"
                score_text = f"\n{partner.correct} correct " \
                             f"answer(s) | {partner.incorrect} incorrect " \
                             f"answer(s)"
                self.question_label.configure(
                    text=answer+score_text, fg="red", wrap=500)
                self.answer_label.config(
                    text=f"The correct answer was {self.key}", fg="black",
                    font="Arial 10 bold")
            # Adding data to list in the main menu
            partner.all_answers.append(answer)
            partner.history_button.configure(state=NORMAL)
    ...
    def next_command(self, mode):
        ...
        self.answer_label.config(
            text=self.answer_label_text, font="Arial 10 italic")
        ...
        self.question_label.configure(
            fg="black", wrap=300,
            text=f"What {self.mode} matches with "
                 f"this {self.anti_mode}?:\n"
